...
const URL = 'http://localhost:8804/api/vehiculos';
...
formA.addEventListener('submit', (e) => {
    ...
    const URLadd = 'http://localhost:8804/api/vehiculos'
    fetch(URLadd,{
        method: 'POST',
...
formE.addEventListener('submit', (e) => {
    ...
    console.log(datosVehiculo);
    const URLEd = `http://localhost:8804/api/vehiculos/${placaE}`
    fetch(URLEd,{
        method: 'PATCH',
        body: JSON.stringify(datosVehiculo),
        headers:{
...
document.addEventListener("click", ({target}) => {
    if(target.classList.contains('btnDel')){
        ...
        if(confirmar){
            const URLDel = `http://localhost:8804/api/vehiculos/${placaDel}`
            fetch(URLDel,{
                method: 'DELETE'
            })
            setTimeout(() => {
